...
function shortener(alphabet, protocol, ids) {
  ...
  function shorten(urll) {
    // TODO
    var digits = toDigits(ids);
    var urlshort = protocol;
    for(var d = 0; d < digits.length; d++){
      urlshort = urlshort + (alphabet[digits[d]]);
    }
  ...
  function expand(urls) {
    // TODO
    var spath = getPath(urls);
    var indeces = [];
    for(var index = 0; index < spath.length; index++){
      indeces.push(alphabet.indexOf(spath[index]));
    }

    var sum = 0;
    var exponent = 0;
    for(var j = indeces.length-1; j >= 0; j--){
      sum += indeces[j] * Math.pow(62, exponent);
      exponent++;
    }
    return urlmap[sum];
  }
